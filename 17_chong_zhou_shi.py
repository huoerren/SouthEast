    ...
    if link not in unfit_urls:
        ...
        if '金额：\n' in content_02:
            ...
            if match_Measure:
                ...
                Measure = re.search('行政处罚：(.*?)', content).group(1)



        #  UnifiedSocialCode  AdministrativeNumber CreateDate   |
        # print([LinkUrl , Name ,Category , According, Measure ,CreateDate])
...
def getLinks():
    for i in range(1,19):
        url = 'http://www.cdepb.gov.cn/cdepbws/Web/Template/GovDefaultList.aspx?cid=856&page=' + str(i)
