...
def getDataFromUrl(link,title):
    ...
    if Name.strip() == '':
        pattern5 = re.compile(r'关于对(.*?)行政处罚')
        match_Name = pattern5.search(doc('.ArticleTitle').text())
        if match_Name:
            Name = match_Name.group(1)
    ...
    if CreateDate.strip() == '':
        if '时间' in content:
            CreateDate = content.split('时间')[1].replace('：','').replace('.','-').replace('年','-').replace('月','-').replace('日','')
...
def getLinks():
    for i in range(1, 16):
        url = 'http://www.cdepb.gov.cn/cdepbws/Web/Template/GovDefaultList.aspx?cid=847&page=' + str(i)
