...
def getDataFromUrl(link,title):
    ...
    if link not in unfit_urls:
        ...
        table = doc('.ArticleContent').children('span').children('table')
        if table.length >0 :
            trs = doc('.ArticleContent').children('span').children('table').children('tbody').children('tr')
            for tr in trs.items():
                if '序号' not in textHandel(tr.text(),1) and textHandel(tr.text(),1).strip() != '':
                    if tr.children('td').length >5:
                        ...
                        Name =     textHandel(tab_val[1] ,1)
                        Category = textHandel(tab_val[2] ,1)
                        According= textHandel(tab_val[3] ,1)
                        Measure =  textHandel(tab_val[4], 1)
                        Measure = '罚款' + Measure + '万元'
                        CreateDate = textHandel(tab_val[5], 1).replace('.', '-')
            ...
            # Category ,According ,Measure ,CreateDate ,UnifiedSocialCode,AdministrativeNumber
            # print([LinkUrl,Name , CreateDate, Category , According  ,Measure ])
...
def getLinks():
    for i in range(1,5):
        url = 'http://www.cdepb.gov.cn/cdepbws/Web/Template/GovDefaultList.aspx?cid=860&page='+ str(i)
